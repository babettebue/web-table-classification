...
def run():
    ...
    # extraction
    warc_paths = sample_random_warc_paths("warc.paths", 50)
...
def extract_tables_from_html(df, sample=None):
    ...
    used_table_ids = pd.read_csv('used_table_ids.csv', sep='\t')

    for _, website in df_s.iterrows():
        try:
            bs = BeautifulSoup(website['html_full'], 'html.parser')
            ...
            if website['url'] in used_table_ids['url'].values:
                logger.info(f"Extracting old table num for {website['url']}...")
                old_table = used_table_ids.loc[used_table_ids['url'] == website['url']].head(1)
                logger.info(f"Found old table with matching url: {old_table}")
                i = old_table['tableNum']
                old_id = old_table['id']
                if website['s3Link'] is not old_table['s3Link']:
                    raise Exception(f'Tables with same URL not found for same s3 Link. Old table id: {old_id}')
            else:
    ...
    df_s = df_s[['url','fullHtmlCode','tableNum', 'oldId', 'htmlCode', 's3Link', 'pageTitle', 'tableText']]
...
def filter_out_layout_tables(df_s):
    ...
    # (compare https://github.com/JulianEberius/dwtc-extractor/blob/master/src/main/java/webreduce/extraction/basic/BasicExtractionAlgorithm.java)
    #
    #
    # * remove tables inside forms
    # * remove tables with sub-tables
    ...
    #
    # not yet:
    # * remove non-regular tables (there is a row with more columns than the most common number)
    ...
    for i, html in df_s.fullHtmlCode.iteritems():
        ...
        #find subtables
        # n_subtables = len(table.find_all("table"))
        # if n_subtables > 0:
        #     filt.append(False)
        #     continue

        #find rowspan
        # n_rowspan = len(table.find_all("rowspan"))
        # if n_rowspan > 0:
        #     filt.append(False)
        #     continue

        #find colspan
        # n_colspan = len(table.find_all("colspan"))
        # if n_colspan > 0:
        #     filt.append(False)
        #     continue
        ...
        #find forms
        # n_forms = len(table.find_all("form"))
        # if n_forms > 0:
        #     filt.append(False)
        #     continue
...
def _merge_files(input_suffix, result_suffix):
    ...
    # persisting
    df_final = df_final[['id','url','fullHtmlCode','tableNum', 'oldId', 'htmlCode', 's3Link', 'pageTitle', 'tableText', 'language', 'languageTitle', 'languageTable']]
