...
funcmodel = VGG16(weights='imagenet',
        include_top=True,
        pooling='avg',
...
model5=Sequential()
model5.add(keras.Input(shape=(224,224,3)))
for layers in funcmodel.layers[1:-4]:
    model5.add(layers)
model5.add(GlobalAveragePooling2D())
model5.summary()

#load fine-tuned weights
#model5.load_weights(r'E:\Babette\MasterThesis\Feature_extractor\VGG16_finetuned\vgg16_finetuned_weights_feature_extractor.h5')
model5.load_weights(r'vgg16_finetuned_weights_feature_extractor.h5')


model1 = Sequential()
model1.add(keras.Input(shape=(224,224,3)))
for layers in model5.layers[:-16]:
    model1.add(layers)
...
for layers in model5.layers[:10]:
    model3.add(layers)
...
df_full.to_pickle(r'manual_vgg16_finetuned_feature_maps_dataset.pkl', protocol=4)
