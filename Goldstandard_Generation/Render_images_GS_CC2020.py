...
# ## Create renderable html text for GS new
#
...
def run():
    ...
    df = pd.read_pickle('final_result_250_warc_files.pkl')
    # prepare html code for rendering
    #####################################################downsample for testing ##########################################################
    # prepare = df.sample(n=20, random_state=1)
    ######################################################################################################################################
    ...
    prepare2.to_pickle('gs_prepare2_renderable_250_warc_files.pkl')
    ...
    #prepare3 = pd.read_pickle('gs_renderable_250_warc_files.pkl')
...
def run_concurrent(func, df, *args, **kwargs):
    ...
    print('Trying to combine all rendering error results into single dataframe.')
